...
`install.sh` 只会在目标 Hermes home 下创建或更新这两个入口：
...
```bash
git clone https://github.com/xllzaaa/hermes-feishu-a2a-plugin.git
...
3. 安装到 Hermes profile：

```bash
PROFILES="product developer" ./install.sh
```
...
4. 每个 Hermes 实例启动前设置自己的身份：

```bash
export HERMES_FEISHU_A2A_REGISTRY_FILE="$PWD/agents.local.json"
...
另一个实例只需要把 `HERMES_FEISHU_SELF_ACCOUNT_ID` 和 profile 换掉：
...
hermes -p developer gateway run
```
...
`agents.local.json` 是所有 Agent 共用的列表，可以在多台机器上复制同一份。
...
- `name`：机器人在协作提示里显示的名字。
- `open_id`：飞书机器人的真实 open_id，用来发送蓝色原生 `@`。
- `account_id`：你自定义的稳定 ID，用来区分当前 Hermes 是哪个 Agent。
- `aliases`：可选别名，写 `@开发` 也能匹配到 `开发Agent`。
...
多主机部署的关键点只有一个：所有机器共享同一份 `agents.local.json` 内容，但每台机器的 `HERMES_FEISHU_SELF_ACCOUNT_ID` 必须不同。
